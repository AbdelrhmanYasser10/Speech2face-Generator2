...
class Generator(nn.Module):
    def __init__(self, input_channel, channels, output_channel):
        ...
        self.model = nn.Sequential(
            ...
            nn.ConvTranspose2d(
                channels[4], output_channel, 1, 1, 0, bias=True),
            nn.Tanh()
        )
    ...
    if train:
        if net_params['model_path'] is not None:
            ...
            net.load_state_dict(torch.load(
                net_params['model_path'], map_location=torch.device('cpu')))
        net.train()
        optimizer = optim.RMSprop(net.parameters(
        ), lr=params['lr'], alpha=0.9, eps=1e-08, weight_decay=0, momentum=0, centered=False)
    else:
        net.eval()
        print('load model from {}'.format(net_params['model_path']))
        net.load_state_dict(torch.load(
