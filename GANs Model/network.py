...
def get_network(net_type, params, train=True):
    ...
    if train:
        ...
        net.eval()
        #net.load_state_dict(torch.load(net_params['model_path'],map_location=torch.device('cpu')),strict=False)
        checkpoint = torch.load(net_params['model_path'], map_location=torch.device('cpu'))['model_state_dict']
